...
class HyperDown
{
    ...
    private function parseCode(array $lines, array $parts)
    {
        ...
        return preg_match("/^\s*$/", $str) ? '' :
            '<pre><code' . (!empty($lang) ? " class=\"language-{$lang} lang-{$lang} {$lang}\"" : '') // Kraity Major Modification
            . (!empty($rel) ? " rel=\"{$rel}\"" : '') . '>'
    ...
    private function parseTable(array $lines, array $value)
    {
        ...
        $html = '<div class="mdui-table-fluid"><table class="mdui-table mdui-table-hoverable">'; // Kraity Major Modification
        $body = $head ? NULL : true;
        $output = false;
        ...
        $html .= '</table></div>'; // Kraity Major Modification
        return $html;
    }
